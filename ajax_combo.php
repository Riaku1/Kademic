...
*/
	...
	$lookups = [
		'enrollment' => [
			'full_name' => [
				'parent_table' => 'registration',
				'parent_pk_field' => 'id',
				'parent_caption' => '`registration`.`full_name`',
				'parent_from' => '`registration` ',
				'filterers' => [],
				'custom_query' => 'SELECT `registration`.`id`, `registration`.`full_name` FROM `registration` ORDER BY 2',
			...
			'class' => [
				...
				'parent_from' => '`classes` ',
				'filterers' => [],
				'custom_query' => 'SELECT `classes`.`id`, `classes`.`class` FROM `classes` ORDER BY 2',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => true,
			],
			'year' => [
				'parent_table' => 'classes',
				'parent_pk_field' => 'id',
				'parent_caption' => '`classes`.`year`',
				'parent_from' => '`classes` ',
				'filterers' => [],
				'custom_query' => 'SELECT `classes`.`id`, `classes`.`year` FROM `classes` ORDER BY 2',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => false,
			],
			'fees_code' => [
				'parent_table' => 'fees_structure',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`fees_structure`.`code`) || CHAR_LENGTH(`fees_structure`.`fees`), CONCAT_WS(\'\', `fees_structure`.`code`, \'/\', `fees_structure`.`fees`), \'\')',
				'parent_from' => '`fees_structure` ',
				'filterers' => [],
				'custom_query' => 'SELECT `fees_structure`.`id`, IF(CHAR_LENGTH(`fees_structure`.`code`) || CHAR_LENGTH(`fees_structure`.`fees`), CONCAT_WS(\'\', `fees_structure`.`code`, \'/\', `fees_structure`.`fees`), \'\') FROM `fees_structure` ORDER BY 2',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => false,
			],
		...
		'fees_payments' => [
			'student' => [
				...
				'parent_pk_field' => 'stid',
				'parent_caption' => 'IF(    CHAR_LENGTH(`registration1`.`full_name`), CONCAT_WS(\'\',   `registration1`.`full_name`), \'\')',
				'parent_from' => '`enrollment` LEFT JOIN `registration` as registration1 ON `registration1`.`id`=`enrollment`.`full_name` LEFT JOIN `classes` as classes1 ON `classes1`.`id`=`enrollment`.`class` LEFT JOIN `fees_structure` as fees_structure1 ON `fees_structure1`.`id`=`enrollment`.`fees_code` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
				'list_type' => 0,
			...
			'class' => [
				'parent_table' => 'enrollment',
				'parent_pk_field' => 'stid',
				'parent_caption' => 'IF(    CHAR_LENGTH(`classes1`.`class`), CONCAT_WS(\'\',   `classes1`.`class`), \'\')',
				'parent_from' => '`enrollment` LEFT JOIN `registration` as registration1 ON `registration1`.`id`=`enrollment`.`full_name` LEFT JOIN `classes` as classes1 ON `classes1`.`id`=`enrollment`.`class` LEFT JOIN `fees_structure` as fees_structure1 ON `fees_structure1`.`id`=`enrollment`.`fees_code` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'term' => [
				'parent_table' => 'enrollment',
				'parent_pk_field' => 'stid',
				'parent_caption' => '`enrollment`.`term`',
				'parent_from' => '`enrollment` LEFT JOIN `registration` as registration1 ON `registration1`.`id`=`enrollment`.`full_name` LEFT JOIN `classes` as classes1 ON `classes1`.`id`=`enrollment`.`class` LEFT JOIN `fees_structure` as fees_structure1 ON `fees_structure1`.`id`=`enrollment`.`fees_code` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
		...
		'staff' => [
			'subject' => [
				...
				'parent_pk_field' => 'sub_id',
				'parent_caption' => '`subjects`.`subject`',
		...
		'assessments' => [
			'class' => [
				'parent_table' => 'enrollment',
				'parent_pk_field' => 'stid',
				'parent_caption' => 'IF(CHAR_LENGTH(`enrollment`.`class`) || CHAR_LENGTH(`enrollment`.`term`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`classes1`.`class`), CONCAT_WS(\'\',   `classes1`.`class`), \'\'), \'-\', `enrollment`.`term`), \'\')',
				'parent_from' => '`enrollment` LEFT JOIN `registration` as registration1 ON `registration1`.`id`=`enrollment`.`full_name` LEFT JOIN `classes` as classes1 ON `classes1`.`id`=`enrollment`.`class` LEFT JOIN `fees_structure` as fees_structure1 ON `fees_structure1`.`id`=`enrollment`.`fees_code` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
			...
			'teacher' => [
				'parent_table' => 'staff',
				'parent_pk_field' => 'id',
				'parent_caption' => '`staff`.`full_name`',
				'parent_from' => '`staff` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`sub_id`=`staff`.`subject` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
				'list_type' => 0,
		...
		'results' => [
			'assess' => [
				...
				'parent_pk_field' => 'id',
				'parent_caption' => '`assessments`.`id`',
				'parent_from' => '`assessments` LEFT JOIN `enrollment` as enrollment1 ON `enrollment1`.`stid`=`assessments`.`class` LEFT JOIN `classes` as classes1 ON `classes1`.`id`=`enrollment1`.`class` LEFT JOIN `staff` as staff1 ON `staff1`.`id`=`assessments`.`teacher` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`sub_id`=`assessments`.`subject` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
				'list_type' => 0,
			...
			'total_marks' => [
				'parent_table' => 'assessments',
				'parent_pk_field' => 'id',
				'parent_caption' => '`assessments`.`total_marks`',
				'parent_from' => '`assessments` LEFT JOIN `enrollment` as enrollment1 ON `enrollment1`.`stid`=`assessments`.`class` LEFT JOIN `classes` as classes1 ON `classes1`.`id`=`enrollment1`.`class` LEFT JOIN `staff` as staff1 ON `staff1`.`id`=`assessments`.`teacher` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`sub_id`=`assessments`.`subject` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
		...
		'rizalts' => [
			'exm_id' => [
				'parent_table' => 'exams',
				'parent_pk_field' => 'exm_id',
				'parent_caption' => '`exams`.`exm_id`',
				'parent_from' => '`exams` LEFT JOIN `enrollment` as enrollment1 ON `enrollment1`.`stid`=`exams`.`class` LEFT JOIN `classes` as classes1 ON `classes1`.`id`=`enrollment1`.`class` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`sub_id`=`exams`.`subject` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
				'list_type' => 0,
			...
			'total_marks' => [
				'parent_table' => 'exams',
				'parent_pk_field' => 'exm_id',
				'parent_caption' => '`exams`.`total_marks`',
				'parent_from' => '`exams` LEFT JOIN `enrollment` as enrollment1 ON `enrollment1`.`stid`=`exams`.`class` LEFT JOIN `classes` as classes1 ON `classes1`.`id`=`enrollment1`.`class` LEFT JOIN `subjects` as subjects1 ON `subjects1`.`sub_id`=`exams`.`subject` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
